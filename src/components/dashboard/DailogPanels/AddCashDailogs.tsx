...
export function AddCashConfirmationDialog({ amount, onClose, user }: AddCashConfirmationDialogProps) {
  return (
    <div className="bg-white text-black h-full w-full max-w-md mx-auto flex flex-col mt-32 p-6">
      {user.transaction_mgs_code?.lastStepText ? (
        <div className="px-4">
          <div className="w-20 h-20 rounded-full bg-red-100 flex items-center justify-center mb-6">
            <X className="w-10 h-10 text-red-500" />
          </div>
          <h2 className="text-xl font-bold mb-4">Add Cash Issue</h2>
          <h3 className="text-gray-600 text-lg font-semibold mb-2">{user.transaction_mgs_code.headerText}</h3>
          <p className="text-gray-600 mb-8">{user.transaction_mgs_code.lastStepText}</p>
